...
export function JupyterNotebookIssueViewer(props: Readonly<JupyterNotebookIssueViewerProps>) {
  ...
  React.useEffect(() => {
    processIssueForJupyterNotebook(data, issue, setRenderedCells);
  }, [issue, data]);
...
/**
 * Processes the notebook data and sets the rendered cells based on the issue.
 * @param {string} data - The JSON string representing the notebook content.
 * @param {Issue} issue - The issue object containing textRange.
 * @param {Function} setRenderedCells - Function to update the rendered cells state.
 */
function processIssueForJupyterNotebook(
  data: string | null | undefined,
  issue: Issue,
  setRenderedCells: (cells: { after: ICodeCell; before: ICodeCell[] } | null) => void,
) {
